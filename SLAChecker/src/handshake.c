...
int main(int argc, char** argv){
    ...
    if(connect<1)
        return 2;
    printf("Testing \"HANDOUT_GITHUBID\" (jaemoon-sim)\n"); 
    int github = handout_github("WRONGWRONG_ADDF");
    
    if(github == 1){
        printf("Authenticating with wrong github-id terminated with error\n");
        close(client_sockfd);
        return 1; 
    }
...
int verify(char* verifiee, char **randnum_result){
    ...
    char buff[1024];
    FILE *fp;
    sprintf(cmdline, "gpg --decrypt %s 2>&1", verifiee);
    fp = popen(cmdline, "r");
    if(fp == NULL) { // ERROR
        perror("gpg_verify");
        return -1;
    ...
    int idx = 0; 
    char* good = "gpg: Good signature from \"Jaemoon Sim <jettijam@gmail.com>\"";
    int good_len = strlen(good);
    char result[1024];
    while(fgets(buff, sizeof(buff), fp) != NULL){
        int buff_len = strlen(buff);
        if(idx == 0){
            strncpy(result, buff, buff_len);
            *randnum_result = result;
        }
        if(idx++ == 2){
            int i;
            for(i = 0 ; i < good_len ; i++){
                if(buff[i] != good[i]){
                    pclose(fp);
                    return -1;
                }
